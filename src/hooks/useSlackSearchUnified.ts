  ...
  const handleSearch = useCallback(
    async (params: SlackSearchParams) => {
      ...
      try {
        ...
        // 全体のMarkdown生成
        const fullMarkdowns = threads.map((thread) => generateSingleThreadMarkdown(thread, userMaps, permalinkMaps))
...
`
  ...
  const threadsMarkdown = threads
    .map((thread) => generateSingleThreadMarkdown(thread, userMaps, permalinkMaps))
    .join('\n\n')
