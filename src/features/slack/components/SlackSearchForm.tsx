...
/**
 * Slack検索フォームコンポーネント
 * Slackのメッセージ・スレッド検索と詳細フィルタリング機能を提供する
 * @param form フォーム状態と操作を含むSlackフォーム結果オブジェクト
 */
export const SlackSearchForm: FC<SlackSearchFormProps> = ({ form }) => {
  return (
    <form onSubmit={form.onSubmit} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
        <button
          ...
          className="w-full inline-flex items-center justify-center py-2.5 px-4 border border-transparent shadow-sm text-sm font-medium rounded-sm text-white bg-slack-primary hover:bg-slack-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slack-primary disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed transition-colors duration-150 ease-in-out"
        >
          {form.isLoading ? (
            <>
              <svg
